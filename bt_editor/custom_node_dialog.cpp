  ...
  _editing(false)
{
  ...
  ui->tableWidget->horizontalHeader()->restoreState(
    settings.value(
      "CustomNodeDialog/header").toByteArray() );
  ...
  connect(
    ui->tableWidget, &QTableWidget::cellChanged,
  ...
  if (name.toLower() == "root") {
    ...
    if (empty_param_name) {
      ...
      ui->labelWarning->setText(
        "Reserved port key: the words \"name\" and \"ID\" should not be used.");
