...
GraphicContainer::GraphicContainer(
  std::shared_ptr<DataModelRegistry> model_registry,
  QWidget * parent)
: QObject(parent),
  ...
  _signal_was_blocked(true)
{
  ...
  connect(
    _scene, &QtNodes::FlowScene::nodeDoubleClicked,
    this, &GraphicContainer::onNodeDoubleClicked);

  connect(
    _scene, &QtNodes::FlowScene::nodeCreated,
    this, &GraphicContainer::onNodeCreated);

  connect(
    _scene, &QtNodes::FlowScene::nodeContextMenu,
    this, &GraphicContainer::onNodeContextMenu);

  connect(
    _scene, &QtNodes::FlowScene::connectionContextMenu,
    this, &GraphicContainer::onConnectionContextMenu);

  connect(
    _scene, &QtNodes::FlowScene::nodeDeleted,
    this, &GraphicContainer::undoableChange);

  connect(
    _scene, &QtNodes::FlowScene::nodeMoved,
    this, &GraphicContainer::undoableChange);

  connect(
    _scene, &QtNodes::FlowScene::connectionDeleted,
    this, &GraphicContainer::undoableChange);

  connect(
    _view, &QtNodes::FlowView::startNodeDelete,
    this, [this]()
    {
  ...
  connect(
    _view, &QtNodes::FlowView::finishNodeDelete,
    this, [this]()
    {
  ...
  connect(
    _scene, &QtNodes::FlowScene::connectionCreated,
...
void GraphicContainer::createSubtree(Node & root_node, QString subtree_name)
{
  ...
  {
    QMessageBox::warning(
      nullptr, "SubTree already exists",
      tr(
        "There is already a SubTree called [%1].\n"
...
void GraphicContainer::onNodeCreated(Node & node)
{
  if (auto bt_node = dynamic_cast<BehaviorTreeDataModel *>( node.nodeDataModel() ) ) {
    connect(
      bt_node, &BehaviorTreeDataModel::parameterUpdated,
      this, &GraphicContainer::undoableChange);

    connect(
      bt_node, &BehaviorTreeDataModel::portValueDoubleChicked,
      this, &GraphicContainer::onPortValueDoubleClicked);

    connect(
      bt_node, &BehaviorTreeDataModel::instanceNameChanged,
      this, &GraphicContainer::undoableChange);
    ...
    if (auto subtree_node = dynamic_cast<SubtreeNodeModel *>( bt_node ) ) {
      ...
      connect(
        subtree_node, &SubtreeNodeModel::expandButtonPushed,
        &(node), [&node, this]()
        {
...
void GraphicContainer::onNodeContextMenu(Node & node, const QPointF &)
{
  ...
  if (auto bt_node = dynamic_cast<BehaviorTreeDataModel *>(node.nodeDataModel())) {
    ...
    connect(
      subtree, &QAction::triggered, this, [this, &node]()
      {
        createSubtree(node);
      });
  }
...
void GraphicContainer::createMorphSubMenu(QtNodes::Node & node, QMenu * nodeMenu)
{
  ...
  if (names_in_category.size() == 0) {
    ...
    for (auto & name: names_in_category) {
      ...
      connect(
        action, &QAction::triggered, this, [this, &node, name]
        {
...
void GraphicContainer::createSmartRemoveAction(QtNodes::Node & node, QMenu * node_menu)
{
  ...
  connect(
    smart_remove, &QAction::triggered,
    this, [this, node_ptr]()
    {
...
{
  Node & new_node = _scene->createNodeAtPos(
    abs_node->model.registration_ID,
    abs_node->instance_name,
    cursor);
  BehaviorTreeDataModel * bt_node =
    dynamic_cast<BehaviorTreeDataModel *>( new_node.nodeDataModel() );
  ...
  _scene->createConnection(
    *abs_node->graphic_node, 0,
    *parent_node, 0);
