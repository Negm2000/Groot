  ...
  _monitor_autoconnect(monitor_autoconnect)
{
  ...
  connect(
    ui->toolButtonConnect, &QToolButton::clicked,
    _monitor_widget, &SidepanelMonitor::on_Connect);

  connect(
    _monitor_widget, &SidepanelMonitor::connectionUpdate,
  ...
  connect(
    arrange_shortcut, &QShortcut::activated,
  ...
  connect(
    _editor_widget, &SidepanelEditor::nodeModelEdited,
    this, &MainWindow::onTreeNodeEdited);

  connect(
    _editor_widget, &SidepanelEditor::addNewModel,
    this, &MainWindow::onAddToModelRegistry);

  connect(
    _editor_widget, &SidepanelEditor::destroySubtree,
    this, &MainWindow::onDestroySubTree);

  connect(
    _editor_widget, &SidepanelEditor::modelRemoveRequested,
    this, &MainWindow::onModelRemoveRequested);

  connect(
    _editor_widget, &SidepanelEditor::addSubtree,
  ...
  connect(
    _editor_widget, &SidepanelEditor::setTabScope,
    this, &MainWindow::onSubtreeSelected);

  connect(
    _editor_widget, &SidepanelEditor::renameSubtree,
  ...
  connect(
    _replay_widget, &SidepanelReplay::loadBehaviorTree,
    this, createSingleTabBehaviorTree);

  connect(
    _replay_widget, &SidepanelReplay::addNewModel,
    this, &MainWindow::onAddToModelRegistry);

  connect(
    ui->toolButtonSaveFile, &QToolButton::clicked,
    this, &MainWindow::on_actionSave_triggered);

  connect(save_shortcut, &QShortcut::activated, this, &MainWindow::on_actionSave_triggered);

  connect(
    _replay_widget, &SidepanelReplay::changeNodeStyle,
    this, &MainWindow::onChangeNodesStatus);

#ifdef ZMQ_FOUND

  connect(
    _monitor_widget, &SidepanelMonitor::addNewModel,
    this, &MainWindow::onAddToModelRegistry);

  connect(
    _monitor_widget, &SidepanelMonitor::changeNodeStyle,
    this, &MainWindow::onChangeNodesStatus);

  connect(
    _monitor_widget, &SidepanelMonitor::loadBehaviorTree,
    this, createSingleTabBehaviorTree);
#endif

  ui->tabWidget->tabBar()->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(
    ui->tabWidget->tabBar(), &QTabBar::customContextMenuRequested,
...
GraphicContainer * MainWindow::createTab(const QString & name)
{
  ...
  connect(
    ti, &GraphicContainer::undoableChange,
  ...
  connect(
    ti, &GraphicContainer::requestSubTreeExpand,
    this, &MainWindow::onRequestSubTreeExpand);

  connect(
    ti, &GraphicContainer::requestSubTreeCreate,
  ...
  connect(
    ti, &GraphicContainer::addNewModel,
    this, &MainWindow::onAddToModelRegistry);
  ...
  try {
    ...
    if (!document.setContent(xml_text, &errorMsg, &errorLine) ) {
      throw std::runtime_error(
              tr("Error parsing XML (line %1): %2").arg(errorLine).arg(
...
{
  QSettings settings;
  QString directory_path = settings.value(
    "MainWindow.lastLoadDirectory",
...
void MainWindow::streamElementAttributes(
...
void MainWindow::recursivelySaveNodeCanonically(
  QXmlStreamWriter & stream,
...
void MainWindow::on_actionSave_triggered()
{
  ...
  QSettings settings;
  QString directory_path = settings.value(
    "MainWindow.lastSaveDirectory",
    QDir::currentPath() ).toString();

  auto fileName = QFileDialog::getSaveFileName(
    this, "Save BehaviorTree to file",
...
{
  QSettings settings;
  QString last_load_path = settings.value(
    "MainWindow.lastLoadDirectory",
    QDir::homePath() ).toString();
  QString directory_path = settings.value(
    "MainWindow.lastSaveSvgDirectory",
...
void MainWindow::onRequestSubTreeExpand(
...
void MainWindow::onModelRemoveRequested(QString ID)
{
  ...
  if (node_found && node_type != NodeType::SUBTREE) {
    QMessageBox::warning(
      ...
      QString(
        "You are using this model in the Tree called [%1].\n"
...
void MainWindow::onCreateAbsBehaviorTree(
  const AbsBehaviorTree & tree,
...
void MainWindow::updateCurrentMode()
{
  ...
  if (_current_mode == GraphicMode::EDITOR) {
    connect(
      ui->toolButtonLoadFile, &QToolButton::clicked,
...
void MainWindow::on_actionReplay_mode_triggered()
{
  ...
  if (currentTabInfo()->scene()->nodes().size() > 0) {
    res = QMessageBox::warning(
      this, tr("Carefull!"),
      tr(
        "If you switch to Log Replay Mode, "
        "the current BehaviorTree in the Scene will be deleted"),
      QMessageBox::Cancel | QMessageBox::Ok, QMessageBox::Cancel);
  }
  if (res == QMessageBox::Ok) {
...
void MainWindow::onChangeNodesStatus(
  const QString & bt_name,
...
void MainWindow::onTabCustomContextMenuRequested(const QPoint & pos)
{
  ...
  connect(
    rename, &QAction::triggered, this, [this, tab_index]()
    {
  ...
  connect(
    set_main, &QAction::triggered, this, [this, tab_index]()
    {
...
void MainWindow::onTabRenameRequested(int tab_index, QString new_name)
{
  ...
  if (getTabByName(new_name) ) {
    QMessageBox::warning(
      ...
      tr(
        "There is already a BehaviorTree called [%1].\n"
...
void MainWindow::on_actionReportIssue_triggered()
{
  ...
  QMessageBox::warning(
    this, "Issue Reporting",
    tr(
      "Reporting an issue you allow us to make this software better and better. Thanks!\n"
