...
public slots:
  ...
  void onCreateAbsBehaviorTree(
    const AbsBehaviorTree & tree,
    const QString & bt_name,
    bool secondary_tabs = true);

  void onChangeNodesStatus(
    const QString & bt_name, const std::vector < std::pair < int,
...
private:
  ...
  void recursivelySaveNodeCanonically(
