  ...
  _style_caption_alias(model.registration_ID)
{
  ...
  for (int pref_index = 0; pref_index < 3; pref_index++) {
    for (const auto & port_it: model.ports) {
      ...
      connect(
        form_field, &GrootLineEdit::doubleClicked,
        this, [this, form_field]()
        {emit this->portValueDoubleChicked(form_field);});

      connect(
        form_field, &GrootLineEdit::lostFocus,
        this, [this]()
  ...
  //--------------------------------------
  connect(
    _line_edit_name, &QLineEdit::editingFinished,
    this, [this]()
    {
...
NodeDataModel::ConnectionPolicy BehaviorTreeDataModel::portOutConnectionPolicy(QtNodes::PortIndex)
const
{
  return ( nodeType() == NodeType::DECORATOR ||
         _model.registration_ID == "Root") ? ConnectionPolicy::One : ConnectionPolicy::Many;
}
