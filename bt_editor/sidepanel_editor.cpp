  ...
  _model_registry(registry)
{
  ...
  connect(
    ui->paletteTreeWidget, &QWidget::customContextMenuRequested,
    this, &SidepanelEditor::onContextMenu);

  connect(
    ui->paletteTreeWidget, &QTreeWidget::itemDoubleClicked,
...
void SidepanelEditor::on_paletteTreeWidget_itemSelectionChanged()
{
  ...
  if (selected_items.size() == 0) {
    ...
    for (const auto & port_it: model.ports) {
      ui->portsTableWidget->setItem(
        row, 0,
        new QTableWidgetItem(QString::fromStdString(toStr(port_it.second.direction))));
    ...
    ui->portsTableWidget->horizontalHeader()->setSectionResizeMode(
      0,
...
void SidepanelEditor::onContextMenu(const QPoint & pos)
{
  ...
  connect(
    edit, &QAction::triggered, this, [this, selected_name]()
    {
  ...
  connect(
    remove, &QAction::triggered, this, [this, selected_name]()
    {
...
void SidepanelEditor::onReplaceModel(
...
void SidepanelEditor::on_buttonUpload_clicked()
{
  ...
  QString directory_path = settings.value(
    "SidepanelEditor.lastSaveDirectory",
...
void SidepanelEditor::on_buttonDownload_clicked()
{
  QSettings settings;
  QString directory_path = settings.value(
    "SidepanelEditor.lastLoadDirectory",
    QDir::homePath() ).toString();

  QString fileName = QFileDialog::getOpenFileName(
    this, tr("Load TreenNodeModel from file"),
    directory_path,
...
NodeModels SidepanelEditor::importFromXML(QFile * file)
{
  ...
  if (!file->open(QIODevice::ReadOnly)) {
    QMessageBox::warning(
      this, "Error loading TreeNodeModel from file",
  ...
  if (xml_root.isNull() || xml_root.tagName() != "root") {
    QMessageBox::warning(
      this, "Error loading TreeNodeModel form file",
