...
void SidepanelReplay::updateTableModel(const AbsBehaviorTree & locaded_tree)
{
  ...
  if (transitions_count > 0) {
    ...
    emit _table_model->dataChanged(_table_model->index(0, 0),
      _table_model->index(transitions_count - 1, 3) );
...
void SidepanelReplay::on_LoadLog()
{
  ...
  QString directory_path = settings.value(
    "SidepanelReplay.lastLoadDirectory",
...
void SidepanelReplay::loadLog(const QByteArray & content)
{
  ...
  if (!valid_tree) {
    QMessageBox::warning(
      this, "Flatbuffer verification failed",
      "Failed to load this file.\n"
  ...
  for (size_t offset = 4 + bt_header_size; offset < content.size(); offset += 12) {
    ...
    transition.prev_status =
      convert(flatbuffers::ReadScalar<Serialization::NodeStatus>(&buffer[offset + 10]));
    transition.status =
      convert(flatbuffers::ReadScalar<Serialization::NodeStatus>(&buffer[offset + 11]));
    ...
    if (transition.prev_status != NodeStatus::IDLE && transition.status == NodeStatus::IDLE) {
      idle_counter++;
    } else if (transition.prev_status == NodeStatus::IDLE &&
      transition.status != NodeStatus::IDLE)
    {
      idle_counter--;
    }
...
void SidepanelReplay::updatedSpinAndSlider(int row)
{
  auto it = std::upper_bound(
    _timepoint.begin(), _timepoint.end(), row,
...
void SidepanelReplay::on_pushButtonPlay_toggled(bool checked)
{
  ...
  if (checked) {
    ...
    ui->tableView->scrollTo(
      _table_model->index(_prev_row, 0),
...
void SidepanelReplay::onPlayUpdate()
{
  ...
  while (_next_row < LAST_ROW - 1 &&
    (_transitions[_next_row + 1].timestamp - _transitions[_next_row].timestamp) <
