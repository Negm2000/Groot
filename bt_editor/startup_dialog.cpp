...
void StartupDialog::updateCurrentMode()
{
  ...
  if (_mode == GraphicMode::EDITOR) {
    ...
    ui->labelDescription->setText(
      "Connect to a remote server to visualize the status"
      " of a BehaviorTree in real-time.");
  } else if (_mode == GraphicMode::REPLAY) {
    ui->labelDescription->setText(
      "Load a BehaviorTree Log file to visualize all the"
      " transitions off-line.");
  }

  ui->toolButtonEditor->setStyleSheet(
    _mode ==
    GraphicMode::EDITOR ? selected_style : default_style);
#ifdef ZMQ_FOUND
  ui->toolButtonMonitor->setStyleSheet(
    _mode == GraphicMode::MONITOR ? selected_style : default_style);
#endif
  ui->toolButtonReplay->setStyleSheet(
    _mode ==
    GraphicMode::REPLAY ? selected_style : default_style);
}
