...
void EditorTest::loadFile()
{
  ...
  QVERIFY2(
    file_xml.simplified() == saved_xml.simplified(),
  ...
  QVERIFY2(
    saved_xml_expanded.simplified() == saved_xml.simplified(),
    "Loaded and saved XMl are not the same");
...
void EditorTest::modifyCustomModel()
{
  ...
  auto pass_window_items = treeWidget->findItems(
    "PassThroughWindow",
    Qt::MatchExactly | Qt::MatchRecursive);
  QCOMPARE(pass_window_items.empty(), true);

  auto jump_window_items = treeWidget->findItems(
    jump_model.registration_ID,
    Qt::MatchExactly | Qt::MatchRecursive);
